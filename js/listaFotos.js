    	...
    	'</li>'
		 ...
		 /*Pagina dinamica*/
		    content = '<div data-role="page"  style="background:#FFFFFF;" id="id_' + index + '" data-url="id_' + index + '" data-theme="a" >' +
					...
					'<a  data-role="button" data-icon="back" onclick="listaFotos()" class="ui-link ui-btn-left ui-btn ui-icon-back ui-btn-icon-left ui-shadow ui-corner-all" role="button">Atrás</a>'+
					
					'<h1>' + item.titulo + '</h1>' +
					'</div>' +
					'<div data-role="content" id="contenido" >' +
					'<img src="http://cienvio.creatactil.com/files/'+item.enlace+'" style="width:100%; "onclick="descargarFoto(\''+item.enlace+'\')">'+
								  '<p>' +
...
function abrirfichero5(ruta5){
	...
	window.open (ruta5, '_system', 'location=no,closebuttoncaption=Close,enableViewportScale=yes');
